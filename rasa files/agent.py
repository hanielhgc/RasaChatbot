...
try:
    with open("C:/Users/Haniel/PycharmProjects/RasaChatbot/dicts/abreviacoes.txt", "r") as a_file:
      for line in a_file:
        stripped_line = line.strip()
        a, b = stripped_line.split("\t")
        abbreviated.append(a)
        full_word.append(b)
except:
    print("Error! Abbreviation file not loaded!")
    ...
    async def handle_message(
        ...
        **kwargs,
        ...
        async with self.lock_store.lock(message.sender_id):
            ...
            message_tokens = word_tokenize(message.text)

            message_tokens_modified = []

            for j in range(len(message_tokens)):
                for i in range(len(abbreviated)):
                    if message_tokens[j] == abbreviated[i]:
                        message_tokens[j] = full_word[i]

            preprocessed_message = ' '.join(message_tokens)
            print(preprocessed_message)
            message.text = preprocessed_message
